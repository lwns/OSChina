...
import com.core.op.lib.utils.AppOperator;
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            addJavascriptInterface(new OnWebViewImageListener() {
                ...
                public void showImagePreview(String bigImageUrl) {
                    if (bigImageUrl != null && !StrUtil.isEmpty(bigImageUrl)) {
    ...
    public void loadDetailDataAsync(final String content, Runnable finishCallback) {
        ...
        if (context != null && context instanceof Activity) {
            ...
            AppOperator.runOnThread(new Runnable() {
                @Override
                public void run() {
                    final String body = setupWebContent(activity, content, true, true, "");
    ...
    public void loadTweetDataAsync(final String content, Runnable finishCallback) {
        ...
        if (context != null && context instanceof Activity) {
            ...
            AppOperator.runOnThread(new Runnable() {
                @Override
                public void run() {
                    final String body = setupWebContent(activity, content, "");
                    activity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            //loadData(body, "text/html; charset=UTF-8", null);
...
//            TLog.e(OWebView.class.getName(), "loadDetailDataAsync error, the Context isn't ok");
    ...
    private static String setupWebContent(Context context, String content, String style) {
...
//        if (AppContext.get(AppConfig.KEY_LOAD_IMAGE, true)||
    ...
    private static String setupWebContent(Context context, String content, boolean isShowHighlight, boolean isShowImagePreview, String css) {
        if (TextUtils.isEmpty(content) || TextUtils.isEmpty(content.trim()))
            return "";
